...
public class BinarySearch {
    ...
    public static void main(String[] args) {
        int[] array = {-998,-997,-995,-994,-991,-986,-984,-982,-981,-980,-978,
                -975,-973,-972,-970,-968,-967,-966,-962,-961,-960,-959,-958,-954,-953,-952,
                -950,-947,-946,-945,-944,-943,-942,-941,-939,-937,-936,-934,-931,-929,-928,-925,-924,-923,
                -922,-921,-918,-917,-916,-914,-912,-910,-909,-908,-905,-900,-897,-896,-895,-894,-893,-892,-891,-886,
                -882,-881,-880,-879,-878,-876,-873,-872,-871,-870,-864,-863,-860,-859,-857,-856,-853,-852,-851,
                -846,-844,-843,-841,-840,-835,-831,-829,-825,-822,-820,-819,-817,-815,-814,-812,-810,-808,-807,
                -804,-801,-800,-798,-797,-794,-793,-792,-790,-787,-786,-785,-782,-781,-778,-777,-776,-773,-772,
                -770,-767,-766,-763,-762,-759,-757,-751,-750,-748,-744,-743,-741,-740,-738,-737,-736,-733,-729,
       ...
       // search(array,0,array.length-1,2);
        //System.out.println(infiniteArray(array, 988));

        System.out.println(infSearch(array, 988));
//        int x = search1(array,0, array.length-1,9);
//        System.out.println(x);
    }

    static void search(int[] array,int si,int ei,int target){
        ...
        if(target > array[mid-1]){
            search(array,mid+1, ei,target);
        }
        if(target < array[mid+1]){
            search(array,si, mid-1, target);
        }
    }

    static int search1(int[] array,int si,int ei,int target){

        int mid = si + (ei-si)/2;
        int returnValue = 0;
        if(target == array[mid]) {
            return returnValue = mid;
        }

        if(si == ei){
            return returnValue = si;
        }

        if(target > array[mid-1]){
            returnValue =  search1(array,mid+1, ei,target);
        }
        if(target < array[mid+1]){
            returnValue =  search1(array,si, mid-1, target);
        }
        return returnValue;
    }
    ...
    static int infiniteArray(int[] array,int target){
        ...
        for (int i = 1; j < array.length; i=i*2) {
            if(array[j]==target) return j;
            if(i < array.length) if(array[i]==target) return i;
            if(i >= array.length && start == 0 && end ==0){
                start = j;
                end = array.length - 1;
                break;
            }
            if(array[j] < target && array[i] > target){
                start = j;
                end = i;
                break;
            }
        ...
        return array[start]!=target?-1:start;
    }

    static int infSearch(int[] array,int target){
        ...
        while (start != end) {
            int mid = start + (end -start)/2;
            if(mid < array.length-1) if(array[mid] == target) return mid;
            //if(start == end) return start;
...
//    static int rotatedArray(int[] array, int target){
//
//   }
}
